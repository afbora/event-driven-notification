...
// queue-by-name with configured weights). idempotencyKey, when non-empty,
// becomes the task id — asynq rejects duplicate enqueues of the same id
// within the 24-hour uniqueness window (CLAUDE.md §3.9 second layer).
func NewProcessNotificationTask(
	...
	if idempotencyKey != "" {
		opts = append(opts, hibikenasynq.TaskID(idempotencyKey), hibikenasynq.Unique(idempotencyWindow24h))
	}
